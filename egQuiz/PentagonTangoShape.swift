...
struct PentagonGraphTangoLabelView: View {
    var label: String
    var index: Int
...
struct PentagonGraphTangoBackgroundShape: Shape {
    func path(in rect: CGRect) -> Path {
        let center = CGPoint(x: rect.midX, y: rect.midY)
...
struct PentagonTangoGraphView: View {
    ...
    var body: some View {
            GeometryReader { geometry in
                ...
                ZStack {
                    PentagonGraphTangoBackgroundShape()
                    ...
                    ForEach(Array(QuizTangoLevel.allCases.enumerated()), id: \.offset) { (i, _) in
                        PentagonGraphTangoLabelView(label: labels[i], index: i)
                    }
                    // 目盛りの数字を表示する
